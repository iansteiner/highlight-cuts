...
-   **Web Interface (`src/highlight_cuts/web.py`)**:
    ...
    -   Replaced Game/Player dropdowns with an interactive selection table:
        -   Lists all Game/Player combinations with clip counts.
        -   Clicking a row selects the game/player and loads clips.
    -   Redesigned clips table:
        -   Removed "Clip #", "Duration", and "Status" columns.
        -   Formatted "Start" and "End" as integers (no decimals).
        -   Added "Notes" column.
    -   Updated Video Selection to support nested directory structure (`team/tournament/game.mp4`):
        -   Replaced single video dropdown with Team -> Tournament -> Game dependent dropdowns.
        -   Updated output filename format to `player_team/tournament_game.mp4`.
    -   Refactored UI into two main cards:
        -   Card 1: Video Selection
        -   Card 2: Google Sheet URL, Game/Player Selection, and Actions (Generate & Clips)
    -   Updated Clips Table:
        -   Formatted timestamps as `MM:SS`.
        -   Changed Notes display from tooltip to plain text column.
    -   Fixed Clip Timing Issue:
        -   Reverted to stream copy (`-c copy`) to debug original issue.
        -   Implemented FFmpeg Debug Logging:
            -   Added "Debug Log" section to UI.
            -   Captures and displays full FFmpeg commands and output.
            -   Added `/debug-log` endpoint.
        -   **Root cause identified: Browser/CDN caching issue**
            -   Multiple files with same basename were being cached incorrectly.
            -   Solution: Added timestamp to output filenames (`tournament_game_20251126_152800.mp4`).
            -   Download endpoint strips timestamp for clean user-facing filenames.
